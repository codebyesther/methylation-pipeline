# This version does not group the timepoints into three categories (Baseline, On-Treatment, Post-Treatment).
# Instead, it plots individual treatment cycle stages on the x-axis.
...
def extract_explicit_timepoint(name):
    if "INNOV" in name:
        return None
...
def make_average_trajectory_plot(df, save_path, time_order):
...
def export_boxplot_summary(df, save_path, time_order):
...
df_full["Timepoint"] = df_full["Sample"].apply(extract_explicit_timepoint)
df_full["Replicate_ID"] = df_full["Sample"].apply(extract_replicate)
df_full = df_full.dropna(subset=["Patient_ID", "Timepoint"])

# === Set timepoint order dynamically ===
time_order = ["Baseline"] + sorted([tp for tp in df_full["Timepoint"].unique() if tp and tp.startswith("C")], key=lambda x: int(x[1:])) + ["Off-Tx"]
...
make_average_trajectory_plot(df_plot, os.path.join(plot_dir, "average_trajectory.png"), time_order)
...
export_boxplot_summary(df_full, os.path.join("output", "boxplot_summary_by_timepoint.csv"), time_order)
