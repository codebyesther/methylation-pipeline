...
# If aligned_globmin80 is 0, retain the original aligned_glob20 values
ratio_df = aligned_glob20.iloc[:, 1:].astype(float).reset_index(drop=True).copy()  # Start with glob20 values as default
aligned_globmin80_values = aligned_globmin80.iloc[:, 1:].astype(float).reset_index(drop=True)

# Replace values where globmin80 is not zero
with pd.option_context('mode.use_inf_as_na', True):  # Treat infinities as NaN
    ratio_df = ratio_df.where(aligned_globmin80_values == 0, 
                              (ratio_df / aligned_globmin80_values) * 100000)
