...
cbar.set_label("Scaled Methylation Fragment Count Ratio", fontsize=14)  # Update label here
...
plt.ylabel("Average Scaled Methylation Fragment Count Ratio", fontsize=14)  # Update label here
...
for patient in tqdm(patient_ids, desc="Generating per-patient plots and matrices"):
    ...
    plt.ylabel("Scaled Methylation Fragment Count Ratio", fontsize=14)  # Update label here
    plt.xlabel("Timepoint", fontsize=14)  # Add this line to set xlabel
    plt.xticks(rotation=0, fontsize=14)  # Increase font size for x-axis tick labels
    plt.yticks(fontsize=14)               # Increase font size for y-axis tick labels
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=14)
...
# Combine individual patient methylation matrix CSV files into one CSV file
patient_matrix_files = [os.path.join(args.output_dir, f"methylation_matrix_{patient}.csv") for patient in patient_ids]
...
combined_matrix.to_csv(os.path.join(args.output_dir, "combined_methylation_matrix.csv"))
