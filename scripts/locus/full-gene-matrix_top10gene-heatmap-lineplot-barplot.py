...
# Barplot for delta values of top 10 genes (Baseline → Post-Treatment)
top_gene_deltas_bp = pd.Series(baseline_post)[top_genes].sort_values()
plt.figure(figsize=(10, 6))
sns.barplot(x=top_gene_deltas_bp.values, y=top_gene_deltas_bp.index, color="darkblue")
plt.axvline(0, color="gray", linestyle="--")
plt.xlabel("Avg Change in Methylation (Post-Tx − Baseline)", fontsize=14)
plt.ylabel("Gene", fontsize=14)
plt.title("Top 10 Genes by Average Methylation Change (Baseline → Post-Treatment)", fontsize=14)
plt.tight_layout()
plt.savefig(os.path.join(args.output_dir, "barplot_top10_gene_deltas_bp.png"))
plt.close()

# Barplot for delta values of top 10 genes (Baseline → On-Treatment)
top_gene_deltas_bo = pd.Series(baseline_on)[top_genes].sort_values()
plt.figure(figsize=(10, 6))
sns.barplot(x=top_gene_deltas_bo.values, y=top_gene_deltas_bo.index, color="darkgreen")
plt.axvline(0, color="gray", linestyle="--")
plt.xlabel("Avg Change in Methylation (On-Treatment − Baseline)", fontsize=14)
plt.ylabel("Gene", fontsize=14)
plt.title("Top 10 Genes by Average Methylation Change (Baseline → On-Treatment)", fontsize=14)
plt.tight_layout()
plt.savefig(os.path.join(args.output_dir, "barplot_top10_gene_deltas_bo.png"))
plt.close()

# Barplot for delta values of top 10 genes (On-Treatment → Post-Treatment)
top_gene_deltas_op = pd.Series(on_post)[top_genes].sort_values()
plt.figure(figsize=(10, 6))
sns.barplot(x=top_gene_deltas_op.values, y=top_gene_deltas_op.index, color="darkred")
plt.axvline(0, color="gray", linestyle="--")
plt.xlabel("Avg Change in Methylation (Post-Tx − On-Treatment)", fontsize=14)
plt.ylabel("Gene", fontsize=14)
plt.title("Top 10 Genes by Average Methylation Change (On-Treatment → Post-Treatment)", fontsize=14)
plt.tight_layout()
plt.savefig(os.path.join(args.output_dir, "barplot_top10_gene_deltas_op.png"))
plt.close()
