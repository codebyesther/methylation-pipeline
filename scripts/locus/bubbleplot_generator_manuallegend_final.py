...
for patient in tqdm(collapsed.columns.levels[0], desc="Generating bubble plots per patient"):
    for chrom in coords_df["Chr"].unique():
        ...
        # Horizontal colorbar below the main axis
        if sc is not None:
            cax = inset_axes(ax_main, width="60%", height="3%", loc="lower center", borderpad=3)
            cbar = plt.colorbar(sc, cax=cax, orientation='horizontal')
            cbar.set_label("Fragment Count")

        # Manually draw bubble size legend
        ax_legend = inset_axes(ax_main, width="18%", height="35%", loc="upper right", borderpad=2)
        ax_legend.axis("off")

        sizes = [1, 15, 150]
        radii = [np.sqrt(size) * 5 for size in sizes]  # matches the plot's scale
        y_positions = [0, 0, 0]

        # Calculate vertical positions manually to prevent overlap
        spacing_factor = 1.4
        y_positions[0] = 0  # smallest bubble at bottom
        y_positions[1] = y_positions[0] + radii[0] + radii[1] * spacing_factor
        y_positions[2] = y_positions[1] + radii[1] + radii[2] * spacing_factor

        for size, y in zip(sizes, y_positions):
            ax_legend.scatter(0.5, y, s=size**0.5 * 50, color="gray", alpha=0.5)
            ax_legend.text(1.1, y, str(size), va="center", fontsize=10)

        # Add title manually
        ax_legend.text(0.5, y_positions[-1] + radii[2] * 1.5, "Bubble Size\n(Fragment Count)", 
