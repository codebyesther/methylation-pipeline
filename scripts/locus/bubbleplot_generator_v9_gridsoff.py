...
for patient in tqdm(collapsed.columns.levels[0], desc="Generating bubble plots per patient"):
    for chrom in coords_df["Chr"].unique():
        ...
        for _, row in subset_df.iterrows():
            ...
            bubble_size = row["value"]**0.5 * 5    # bubble size inside the plot area
            sc = ax_main.scatter(
                row["Midpoint"],
                y,
                s=bubble_size,
                ...
                vmax=1000000    # increased upper bound of color scale
            )
        ...
        fig.subplots_adjust(left=0.1, right=0.96, top=0.9, bottom=0.1, wspace=0.2, hspace=0.6)  # left margin of the legend, wspace between the plot area and legend area, hspace controls the vertical spacing between colorbar and legend
...
for chrom in tqdm(coords_df["Chr"].unique(), desc="Generating bubble plots per chromosome"):
    ...
    fig.subplots_adjust(left=0.1, right=0.96, top=0.9, bottom=0.1, wspace=0.2, hspace=0.6)  # Adjusted left margin, wspace between the plot area and legend area, hspace controls the vertical spacing between colorbar and legend
