...
for patient in tqdm(collapsed.columns.levels[0], desc="Generating bubble plots per patient"):
    for chrom in coords_df["Chr"].unique():
        ...
        for size, pos in zip(sizes, positions):
            ...
            ax_legend.text(legend_x_coord + size**0.5 * 1.2, pos, str(size), verticalalignment='center', horizontalalignment='center', fontsize=14)    # Adjust fragment count text position relative to the gray bubbles based on legend_x_coord
