...
for patient in patients:
    ...
    # Line plot (with debug)
    print(f"\n🧬 Patient: {patient}")
    print("📊 Raw timepoints in matrix:", grouped.columns.tolist())
    print("🧪 First few rows of grouped matrix:")
    print(grouped.head())

    grouped = grouped[[tp for tp in tp_order if tp in grouped.columns]]
    if grouped.shape[1] < 2:
        print(f"⚠️ Skipping lineplot for {patient}: Only {grouped.shape[1]} timepoint(s) present.")
    else:
        print("✅ Generating lineplot with timepoints:", grouped.columns.tolist())
